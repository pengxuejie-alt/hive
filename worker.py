...
def get_safe_val(obj, path, default=0):
    try:
        parts = path.split(".")
        val = obj
        for part in parts: val = getattr(val, part)
...
def get_market_data_smart(tickers):
    ...
    for t in valid_tickers:
        try:
            ...
            chain = client_poly.list_snapshot_options_chain(t, params={"limit": 100})
            buckets = {"short": [], "mid": [], "long": []}
            
            for o in chain:
                try:
                    expiry_str = o.ticker[5:11]
            ...
            log(f"✅ {t} 采样完成")
        except Exception as e: log(f"⚠️ {t} 异常: {e}")
    return context
...
def patrol_and_evolve():
    ...
    for d in res.data:
        try:
            ...
            log(f"✅ {d['name']} 演化完毕")
        except Exception as e: log(f"❌ {d['name']} 异常: {e}")
