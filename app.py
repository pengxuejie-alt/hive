...
def fetch_nectar(ticker, needs_options=True):
    ...
    双重寻价逻辑：
    1. 尝试 Snapshot 现价
    2. 若盘前/盘后现价为0，穿透至 Last Trade (最新成交)
    3. 若仍无，退守 Prev Day Close (昨收)
    """
    try:
        ...
        if needs_options and price > 0:
            ...
            data["期权动态"] = f"核心合约数: {len(opts)}"
        return data
    except: return {"代码": ticker, "现价": 0, "状态": "接口受限"}
...
def execute_worker_cycle(d, status_container):
    ...
    try:
        with status_container:
            ...
            prompt = f"你是工蜂{d['name']}。基因:{d['persona']}。现金:{d['balance']}。持仓:{json.dumps(d.get('positions'))}。行情:{json.dumps(nectar_data, ensure_ascii=False)}。返回JSON格式交易指令。"
            
            decision = call_gemini_with_retry(prompt)
            ...
            st.success(f"✅ 任务完成 ({time.time()-t_start:.1f}s)")
            return True
    except Exception as e:
...
with h1:
    ...
    st.caption(f"🧠 当前中枢: `{ACTIVE_BRAIN}` | 状态: 实时演化中")
with h2:
    st.write(" ") # 占位对齐
...
with tabs[1]:
    st.subheader("👑 蜂后基因工程")
    instr = st.text_area("输入孵化指令 (例如：孵化一只专注于 SPY 避险的防御型工蜂):")
    if st.button("开始注入基因并孵化"):
        with st.spinner("基因注入中，请稍候..."):
            try:
                p = f"设计JSON：{{'name':'','logic':'','persona':'','portfolio':['GLD']}}。内容中文。指令：{instr}"
                item = call_gemini_with_retry(p)
