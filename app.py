...
def safe_brain_decision(prompt):
    """
    对抗 429 的核心防御层。
    即使是付费账户，遇到突发并发也会报错，这里强制进行指数级等待重试。
    """
    for attempt in range(3):
        try:
            ...
            if "429" in str(e) and attempt < 2:
                # 遇到熔断，递增等待时间: 3s -> 6s
                wait = (attempt + 1) * 3
                st.toast(f"⏳ 神经中枢拥堵，等待 {wait} 秒后重试...", icon="🧠")
                time.sleep(wait)
                continue
            raise e
...
with h1:
    ...
    st.caption(f"🧠 中枢: `{ACTIVE_BRAIN}` | 🛡️ 抗熔断机制已激活")
with h2:
    st.write(" ")
